...
--   - título do filme;
--   - data do aluguel.
 select f.titulo, a.data_aluguel from filmes as f inner join alugueis as a on f.id_filme = a.id_filme;
...
--   - título do filme;
--   - nome do ator.
select f.titulo, ato.nome_ator from filmes as f
inner join atuacoes as a on a.id_filme = f.id_filme 
inner join atores as ato on ato.id_ator = a.id_ator;
...
inner join atores as a where nacionalidade = 'EUA';
...
inner join atuacoes as atu on a.id_ator = atu.id_ator
inner join filmes as f on atu.id_filme = f.id_filme where ano_lancamento >= 2010 order by ano_lancamento desc;
...
-- A nota não pode ser NULL.
 select c.nome_cliente, f.titulo, a.nota, a.data_aluguel from clientes as c 
 inner join alugueis as a on c.id_cliente = a.id_cliente
 inner join filmes as f on a.id_filme = f.id_filme where  a.nota is not null;
...
--   INNER JOIN
--   WHERE
select c.nome_cliente, f.titulo, a.nota from clientes as c 
inner join alugueis as a on c.id_cliente = a.id_cliente
inner join filmes as f on f.id_filme = a.id_filme where nota = 10;
